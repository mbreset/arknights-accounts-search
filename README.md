...
오퍼를 선택한 뒤 태그 안의 `잠재` 메뉴에서도 제한 없음/2잠 이상~6잠을 바로 선택할 수 있습니다. 선택한 조건은 태그 안에 표시됩니다. 직접 입력과 AND 재선택 방식도 그대로 사용할 수 있습니다. 메뉴는 키보드 방향키/Enter/Escape와 모바일 터치를 지원합니다. `tests/operator-search-mobile.html`은 가상 카탈로그로 390px 화면 배치를 확인하는 테스트 페이지입니다.
