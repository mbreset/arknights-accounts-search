...
검색창 옆에 항상 보이는 `잠재 검색` 체크박스를 켜면 태그 안의 `잠재` 메뉴로 제한 없음/2잠 이상~6잠을 선택할 수 있습니다. 모바일에서는 검색창 바로 위에 표시되며, 별도 옵션 메뉴를 열 필요가 없습니다. 기본은 미선택이고, 지정된 값은 `2+`~`6+`로 짧게 표시됩니다. 체크를 꺼도 검색 조건은 유지되고 태그에 작은 `2+` 표시가 남습니다. 직접 입력과 AND 재선택 방식도 그대로 사용할 수 있습니다. 메뉴는 키보드 방향키/Enter/Escape와 모바일 터치를 지원합니다. `tests/operator-search-mobile.html`은 가상 카탈로그로 390px 화면 배치를 확인하는 테스트 페이지입니다.
